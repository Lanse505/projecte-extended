...
public class CrushingRecipeMapper implements IRecipeTypeMapper {

  @Override
  public String getName() {
    return "Crushing Recipe";
  }

  @Override
  public String getDescription() {
    return "Maps Create Crushing Recipes";
  }
  ...
  public boolean handleRecipe(IMappingCollector<NormalizedSimpleStack, Long> mapper, Recipe<?> recipe, INSSFakeGroupManager inssFakeGroupManager) {
    if (!(recipe instanceof CrushingRecipe crushing)) {
      return false;
    }
    boolean handled = false;
    NonNullList<Ingredient> inputs = crushing.getIngredients();
    NonNullList<ProcessingOutput> outputs = crushing.getRollableResults().stream().filter(output -> output.getChance() >= 1f).collect(Collectors.toCollection(NonNullList::create));
