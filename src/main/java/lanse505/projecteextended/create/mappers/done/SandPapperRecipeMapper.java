...
public class SandPapperRecipeMapper implements IRecipeTypeMapper {

  @Override
  public String getName() {
    return "Sandpaper Polishing Recipe";
  }

  @Override
  public String getDescription() {
    return "Maps Create Sandpaper Polishing Recipes";
  }
  ...
  public boolean handleRecipe(IMappingCollector<NormalizedSimpleStack, Long> mapper, Recipe<?> recipe, INSSFakeGroupManager inssFakeGroupManager) {
    if (!(recipe instanceof SandPaperPolishingRecipe basin)) {
      return false;
    }
    boolean handled = false;
    NonNullList<Ingredient> inputs = basin.getIngredients();
    NonNullList<ProcessingOutput> outputs = basin.getRollableResults().stream().filter(output -> output.getChance() >= 1f).collect(Collectors.toCollection(NonNullList::create));
