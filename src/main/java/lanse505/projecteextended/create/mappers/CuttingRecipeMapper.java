...
public class CuttingRecipeMapper implements IRecipeTypeMapper {

  @Override
  public String getName() {
    return "Cutting Recipe";
  }

  @Override
  public String getDescription() {
    return "Maps Create Cutting Recipes";
  }
  ...
  public boolean handleRecipe(IMappingCollector<NormalizedSimpleStack, Long> mapper, Recipe<?> recipe, INSSFakeGroupManager inssFakeGroupManager) {
    ...
    NonNullList<Ingredient> inputs = cutting.getIngredients();
    NonNullList<ProcessingOutput> outputs = cutting.getRollableResults().stream().filter(output -> output.getChance() >= 1f).collect(Collectors.toCollection(NonNullList::create));
